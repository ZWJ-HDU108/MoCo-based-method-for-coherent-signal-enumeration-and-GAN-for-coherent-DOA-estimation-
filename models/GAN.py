...
class GAN_generator(nn.Module):
    def __init__(self,
        ...
        for num in enc_blk_nums:
            ...
            chan = chan * 2  # 这里是因为每次下采样后通道数需要翻倍
        ...
        for num in dec_blk_nums:
            self.ups.append(
                nn.Sequential(
                    ...
                    nn.PixelShuffle(2)  # 像素重排操作，用于实现高效的上采样
                )
            )
            chan = chan // 2  # 每次上采样后通道数要减半
        ...
        self.padder_size = 2 ** len(self.encoders)  # 为check_image_size服务用的
    ...
    # 当输入数据维度无法被padder_size整除的时候，需要做一定填充
